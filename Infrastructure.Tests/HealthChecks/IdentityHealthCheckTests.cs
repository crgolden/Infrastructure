...
public sealed class IdentityHealthCheckTests
{
    ...
    {
        var check = new IdentityHealthCheck(BuildThrowingClient(new HttpRequestException("timeout")), GetDefaultConfiguration());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Identity", check, null, null) };
    ...
    private static IConfiguration GetDefaultConfiguration() =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { ["OidcAuthority"] = "https://crgolden-identity.azurewebsites.net" })
            .Build();
