...
public sealed class HomeAssistantHealthCheckTests
{
    ...
    {
        var check = new HomeAssistantHealthCheck(BuildClient(HttpStatusCode.ServiceUnavailable), GetDefaultOptions());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Home Assistant", check, null, null) };
    ...
    private static IOptions<ServiceEndpointOptions> GetDefaultOptions() =>
        Options.Create(new ServiceEndpointOptions { HomeAssistant = new Uri("https://deeprog.servehttp.com:8123") });
