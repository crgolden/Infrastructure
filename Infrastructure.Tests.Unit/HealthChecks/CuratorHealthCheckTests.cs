...
public sealed class CuratorHealthCheckTests
{
    ...
    {
        var check = new CuratorHealthCheck(BuildClient(HttpStatusCode.OK, "Healthy"), GetDefaultConfiguration());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Curator", check, null, null) };
    ...
    {
        var check = new CuratorHealthCheck(BuildClient(HttpStatusCode.OK, "Degraded"), GetDefaultConfiguration());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Curator", check, null, null) };
    ...
    {
        var check = new CuratorHealthCheck(BuildThrowingClient(new HttpRequestException("timeout")), GetDefaultConfiguration());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Curator", check, null, null) };
    ...
    private static IConfiguration GetDefaultConfiguration() =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { ["CuratorApiAddress"] = "https://crgolden-curator.azurewebsites.net" })
            .Build();
