...
public sealed class ChurchesHealthCheckTests
{
    ...
    {
        // Arrange
        var check = new ChurchesHealthCheck(BuildClient(HttpStatusCode.OK, SiblingAppHealthCheck.HealthyBody), GetDefaultConfiguration());
        var context = HealthCheckContexts.Create(check, HealthCheckNames.Churches);
    ...
    {
        // Arrange
        var check = new ChurchesHealthCheck(BuildClient(HttpStatusCode.OK, TestValues.NewUnexpectedHealthBody()), GetDefaultConfiguration());
        var context = HealthCheckContexts.Create(check, HealthCheckNames.Churches);
    ...
    {
        // Arrange
        var check = new ChurchesHealthCheck(BuildClient(HttpStatusCode.ServiceUnavailable, string.Empty), GetDefaultConfiguration());
        var context = HealthCheckContexts.Create(check, HealthCheckNames.Churches);
    ...
    private static IConfiguration GetDefaultConfiguration() =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { [ChurchesHealthCheck.ConfigurationKey] = TestValues.NewServiceAddress() })
            .Build();
