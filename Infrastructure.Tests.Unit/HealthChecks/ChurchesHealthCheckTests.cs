...
public sealed class ChurchesHealthCheckTests
{
    ...
    {
        // Arrange
        var check = new ChurchesHealthCheck(BuildClient(HttpStatusCode.OK, Generated.NewUnexpectedHealthBody()), GetDefaultConfiguration());
        var context = HealthCheckContexts.Create(check, HealthCheckNames.Churches);
    ...
    private static IConfiguration GetDefaultConfiguration() =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { [ChurchesHealthCheck.ConfigurationKey] = Generated.NewServiceAddress() })
            .Build();
