...
public sealed class IdentityHealthCheckTests
{
    ...
    {
        // Arrange
        var check = new IdentityHealthCheck(BuildClient(HttpStatusCode.OK, Generated.NewUnexpectedHealthBody()), GetDefaultConfiguration());
        var context = HealthCheckContexts.Create(check, HealthCheckNames.Identity);
    ...
    private static IConfiguration GetDefaultConfiguration() =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { [IdentityHealthCheck.ConfigurationKey] = Generated.NewServiceAddress() })
            .Build();
