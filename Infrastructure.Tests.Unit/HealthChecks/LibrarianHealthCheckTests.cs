...
public sealed class LibrarianHealthCheckTests
{
    ...
    {
        // Arrange
        var check = new LibrarianHealthCheck(BuildClient(HttpStatusCode.OK, Generated.NewUnexpectedHealthBody()), GetDefaultConfiguration());
        var context = HealthCheckContexts.Create(check, HealthCheckNames.Librarian);
    ...
    private static IConfiguration GetDefaultConfiguration() =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { [LibrarianHealthCheck.ConfigurationKey] = Generated.NewServiceAddress() })
            .Build();
