...
public sealed class LibrarianHealthCheckTests
{
    ...
    {
        var check = new LibrarianHealthCheck(BuildClient(HttpStatusCode.OK, "Healthy"), GetDefaultConfiguration());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Librarian", check, null, null) };
    ...
    {
        var check = new LibrarianHealthCheck(BuildClient(HttpStatusCode.OK, "Degraded"), GetDefaultConfiguration());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Librarian", check, null, null) };
    ...
    {
        var check = new LibrarianHealthCheck(BuildThrowingClient(new HttpRequestException("timeout")), GetDefaultConfiguration());
        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("Librarian", check, null, null) };
    ...
    private static IConfiguration GetDefaultConfiguration() =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { ["LibrarianServerAddress"] = "https://crgolden-librarian.azurewebsites.net" })
            .Build();
