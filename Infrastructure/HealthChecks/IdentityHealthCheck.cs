...
public sealed class IdentityHealthCheck : IHealthCheck
{
    ...
    public IdentityHealthCheck(HttpClient httpClient, IConfiguration configuration)
    {
        _httpClient = httpClient;
        var baseUri = configuration.GetValue<Uri?>("OidcAuthority") ?? throw new InvalidOperationException("Invalid 'OidcAuthority'.");
